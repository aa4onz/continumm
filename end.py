...
async def exec(tgt):
    ...
    emb = d.Embed(title=f"Race Finished — #{ch.name}", color=d.Color.red(), description=f"**pace:** `{pace} /hr`\n**counts:** `{cnt}`")
    mvps = [uid for uid, _ in sorted(rc.get("players", {}).items(), key=lambda x: x, reverse=True)[:2]]
    if cnt > 0:
        db_top.insert_one({"pace": pace, "total_counts": cnt, "channel_name": ch.name, "mvp1_id": (mvps if len(mvps)>0 else None), "mvp2_id": (mvps if len(mvps)>1 else None), "timestamp": dt.now(tz.utc)})
