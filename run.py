...
async def exec(tgt):
    ...
    cid = str(chan.id)
    if cid not in ch:
        return await (resp.send("❌ can only be initialized inside og and classic") if isinstance(tgt, c.Context) else resp.send_message("❌ can only be initialized inside og and classic", ephemeral=True))
